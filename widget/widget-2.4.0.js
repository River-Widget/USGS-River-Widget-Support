/*!
 * USGS River Widget v2.4.0 — embeddable live river conditions
...
(function () {
  ...
  function css(cfg, scope) {
    ...
    var s = scope + '{display:block;font-family:' + font + ';color:' + t.fg + ';font-size:inherit;'
      ...
      + (cols
        // A fixed count uses width rather than flex-basis: IE11 miscomputes
        // calc() inside flex-basis but handles it fine in width.
        ? scope + ' .rw-cell{-webkit-box-flex:0;-ms-flex:0 0 auto;flex:0 0 auto;'
          + 'width:calc(100% / ' + cols + ');padding:7px;min-width:0}'
          // Four columns in a phone-width sidebar is not a layout. Media
          // queries inside a shadow root still measure the viewport, which is
          // the right thing to measure here.
          + '@media (max-width:560px){' + scope + ' .rw-cell{width:100%}}'
      ...
      + scope + ' .rw-src{white-space:nowrap}'
      + scope + ' .rw-src-label{margin:0 4px 0 6px}'
      ...
      + scope + ' .rw-station{font-variant-numeric:tabular-nums}'
