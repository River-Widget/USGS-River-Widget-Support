/*!
 * USGS River Widget v2.6.0 — embeddable live river conditions
...
(function () {
  ...
  function css(cfg, scope) {
    ...
    var s = scope + '{display:block;font-family:' + font + ';color:' + t.fg + ';font-size:inherit;'
      ...
      + 'font-size:11px;color:' + t.muted + ';line-height:1.5;-ms-flex:none;flex:none}'
      ...
      + scope + ' .rw-when{white-space:nowrap}'
      // And the provisional notice starts the next line rather than trailing
      // the timestamp, which is where the eye expects the second thought.
      + scope + ' .rw-prov{display:block;margin-top:2px}'
      ...
      // Not nowrap. Holding the whole credit on one line pushed it past the
      // edge of a narrow card, where overflow:hidden then clipped it to
      // "Sta…" — a sidebar, a three-column grid or a phone all hit this.
      // Only the number and its icon are kept together.
      + scope + ' .rw-src{display:inline}'
