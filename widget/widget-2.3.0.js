/*!
 * USGS River Widget v2.3.0 — embeddable live river conditions
...
(function () {
  ...
  function cardHTML(g, cfg, label, source) {
    ...
    // A visitor has no idea what a "fallback" is, so the notice says what it
    // means for them — this is not live, and here is the day it is from.
    if (source === 'cache') {
      html += '<span class="rw-cached">Cached data'
        + (newest ? ' from ' + esc(mdY(newest, cfg.tz || g.tz)) : '') + '</span>';
    }
